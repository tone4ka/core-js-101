...
function doRectanglesOverlap(rect1, rect2) {
  const topMax = Math.max(rect1.top, rect2.top);
  const leftMax = Math.max(rect1.left, rect2.left);
  const bottomMin = Math.min((rect1.top + rect1.height), (rect2.top + rect2.height));
  const rightMin = Math.min((rect1.left + rect1.width), (rect2.left + rect2.width));
  if (bottomMin > topMax && rightMin > leftMax) return true;
  return false;
}
...
function getDigitalRoot(num) {
  const res = String(num).split('').reduce((sum, item) => sum + +item, 0);
...
function toNaryString(num, n) {
  ...
  let numb = num;
  while (numb > 0) {
    res.push(numSys[numb % n]);
    numb = Math.floor(numb / n);
  }
